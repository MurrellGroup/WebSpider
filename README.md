...
Terminal tabs open in **Direct** mode, where the browser keyboard talks straight to xterm and the PTY. On any primary Master or Sub-Spider terminal, **Text box** sends a durable WebSpider message directly to that selected agent; Enter sends and Shift+Enter inserts a newline. WebSpider latches the modifier for the complete physical Enter press and explicitly restores Codex's enhanced keyboard protocol after reconnects, so a shifted Enter cannot degrade into a send during key repeat or a long-running session. Unsent drafts are isolated per terminal and survive in-app navigation and page reloads in that browser tab. On auxiliary shell tabs the composer writes drafted text to the PTY, observing bracketed-paste mode only when the running program enables it. **Refresh** rebuilds xterm from the authoritative terminal log, recalculates rows and columns, and forces a redraw after an awkward resize/window restore; the portal also runs a lightweight repair automatically on focus, visibility, view-mode, and text-box changes.
...
- Automatically managed Codex profiles run noninteractively with `approval_policy=never` and `sandbox_mode=danger-full-access`; this matches WebSpider's existing PTY host-user trust boundary and prevents an unavailable Linux user-namespace sandbox from turning durable work into an unattended approval loop. Each launch also marks only its registered canonical project root as trusted through a session-scoped CLI override, preventing a new-folder trust dialog from silently blocking the managed agent. Explicit profile arguments are otherwise preserved.
- WebSpider never opts a managed Codex session into a different model. It suppresses Codex's rate-limit model nudge and submits durable Text-box, reminder, hook, and agent-to-agent messages with a dedicated `Ctrl+Enter` composer binding that cannot accept a choice popup. A session launched by an older WebSpider is never sent the unsafe fallback `Enter`: its automated messages remain durably queued until it is restarted/resumed with the protection active. Direct terminal users may still deliberately use `/model` or accept a visible Codex choice.
...
- **Files → Connect Overleaf** initializes an empty project directory from a hosted Overleaf Git project or adds a token-free `overleaf` remote to an existing repository. A node-local encrypted credential performs explicit fetch, clean fast-forward pull, and guarded push operations; synchronization is never automatic. The LaTeX workspace compares the current working copy with the last fetched Overleaf state, including incoming, outgoing, uncommitted, and potentially overlapping files.
- Overleaf Git synchronizes manuscript sources rather than its generated PDF. The LaTeX workspace can compile with node-local `latexmk` or `tectonic`, follows the durable command until it finishes, and opens the resulting PDF automatically. **Full screen** hides the rest of the portal for a distraction-free source/PDF workspace. Its compact top review strip can tag several disjoint passages, keep them highlighted, and send one instruction covering all of them; agent proposals still return as individually accepted or rejected diff hunks.
- Desktop dividers resize the project navigator, Attention rail, Files browser, LaTeX source/PDF split, Terminal/Maths split, Notes list, chat sidebars, and Overleaf comparison. Sizes persist in that browser. Attention and the Files browser also collapse without changing any running process.
...
- If a Hub database is lost while remote agents remain alive, reconnecting their existing project directories to a rebuilt Hub exposes the unknown live processes under **Nodes → Surviving agents**. Replacement launch is held to avoid duplicate work; the owner explicitly maps a process to an inactive same-node project agent. Claiming preserves its PTY, log, workspace, and node-local managed Codex context, rotates/re-patches scoped control, and sends an audited recovery notice. It does not reconstruct old Hub threads/tasks or import an already-stopped session, and it is not automatic HA/failover.
