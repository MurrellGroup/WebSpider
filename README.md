...
- Sub-Spiders are first-class user-facing project agents, not merely hidden executors. The user can message and steer each one directly. They receive project-specific instructions and a self-confined credential for durable status, detached commands in their own workspace, and hooks to self or Master. They cannot list the portfolio, command peers, edit policy, or access owner APIs.
