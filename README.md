...
- Overleaf Git synchronizes manuscript sources rather than its generated PDF. The LaTeX workspace can compile with node-local `latexmk` or `tectonic`, follows the durable command until it finishes, and opens the resulting PDF automatically. The agent-selection action remains floating while the manuscript scrolls.
