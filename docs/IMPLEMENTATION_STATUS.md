...
| Direct terminal interaction | Complete for local vertical slice | The selected agent opens on its persistent login shell. The user launches and controls Codex directly; conversation and operational views remain secondary. |
...
| Main-agent scoped control | Complete | The materialized helper supports explicit-request policy changes, usage observations, registered-agent discovery, and durable provenance-preserving messages to another agent. Workers receive no token. |
