...
| Hub-loss live-agent recovery | Minimal owner-assisted continuity | After re-enrollment into a rebuilt Hub, unknown live node processes are quarantined as recovery candidates and replacement launch is held. Nodes lets the owner explicitly map a candidate to an inactive same-node project agent; WebSpider remaps the PTY/log stream, preserves its managed context for later resumes, rotates/re-patches scoped control, audits the claim, and sends one notice. Lost Hub history and stopped-session import are not recovered; backup/restore, standby Hub, and direct node rescue UI remain future work. |
...
| Multi-view terminal | Complete | Bundled xterm.js ANSI/VT rendering, direct browser keyboard input, selection-aware browser copy, Codex/Kitty enhanced-key translation, automatic lightweight refit plus manual authoritative-log rebuild/redraw after window changes, browser-to-PTY resize propagation, live/snapshot watch path, and one controller lease with stale-epoch rejection. |
| Resizable workspaces | Complete | Browser-persistent draggable dividers cover the major desktop side-by-side layouts. Attention and Files can collapse independently; LaTeX has a distraction-free full-screen source/PDF workspace. |
| Multi-passage LaTeX review | Complete | A compact sticky workspace strip tags and highlights disjoint source passages, sends one durable agent request for the set, and retains independent per-hunk approval of the resulting proposal. |
| Managed Codex launch stability | Complete for Codex 0.150.1+ | Rate-limit model-switch nudges are disabled at launch. Automated durable delivery uses a composer-only Ctrl+Enter binding that cannot confirm choice popups; messages for older live sessions remain queued until protected resume. Each launch trusts only its registered canonical project root through a process-scoped override, preventing new-folder trust prompts without changing user configuration. Direct user model choices remain available. |
...
| Scoped agent control | Complete | Main tokens support portfolio orchestration, explicit-request policy changes, and usage observations. Worker tokens remain self-confined. Both are bound to the exact managed process lifecycle, survive Hub reconnects, and are revoked when that runtime ends or is replaced. |
...
| Team chat | Complete | Topic-scoped human links, clipboard/file attachments, SQLite + JSONL logs, explicit all/source/topic agent links, mention-triggered replies, and chat-only WebSpider federation with local remote-message caching. |
| Terminal composer | Complete | Direct xterm input remains available. On every primary Master or Sub-Spider terminal, the Text box sends a durable message to that selected agent; on auxiliary shells it observes bracketed-paste mode before sending to the PTY. Enter sends and Shift+Enter inserts a newline. Shift state is latched through the complete Enter press, and Codex terminals recover enhanced keyboard semantics on every browser attachment. Unsent drafts are terminal-scoped and survive navigation/reload in the current browser tab. |
