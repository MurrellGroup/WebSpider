...
| Managed Codex model stability | Complete for Codex 0.150.1+ | Rate-limit model-switch nudges are disabled at launch. Automated durable delivery uses a composer-only Ctrl+Enter binding that cannot confirm choice popups; messages for older live sessions remain queued until protected resume. Direct user model choices remain available. |
