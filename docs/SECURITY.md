...
14. Team-chat human links and WebSpider federation credentials are distinct revocable high-entropy tokens, independently scoped to all or selected topics; neither authenticates owner, node, terminal, workspace, or orchestration APIs. Agent chat access is a separate explicit per-agent link. Routine chat does not wake agents; exact mentions become idempotent durable deliveries. Messages are recorded in SQLite and append-only private JSONL. Attachments are bounded, checksum-recorded mode-`0600` files in one mode-`0700` Hub directory. Only inert raster images and PDFs are eligible for inline serving; active formats download under a sandbox policy. Public posting is rate-limited per credential.
...
- Automatically managed Codex profiles use noninteractive approval mode and `danger-full-access` unless the profile has explicit arguments. Every managed Codex profile, including an explicit one, receives non-overridable model-stability settings: rate-limit switch nudges are hidden and automated delivery submits with a composer-only Ctrl+Enter binding rather than a popup-accepting Enter. At launch, WebSpider also applies Codex's `trusted` setting to only the agent's registered canonical root through a process-scoped CLI override; it does not alter the user's persistent Codex configuration or trust sibling directories. Older live sessions receive queued messages only until protected resume. This avoids approval and folder-trust deadlocks and accidental WebSpider-triggered model changes, but it is not an OS sandbox: enroll only machines and host-user accounts on which the main agent is authorized to execute work.
