...
- For an auto-detected Codex executable, WebSpider creates a private per-agent Codex home containing the effective `AGENTS.md`. Existing Codex configuration is linked from the same host-user account, excluding instruction, session, log, and temporary directories. This does not create a new OS permission boundary; the agent already executes as that host user.
