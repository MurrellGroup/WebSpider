...
8. Orchestration and behavior-control authority is issued only to a main-role agent. Its short-lived token is accepted only by dedicated portfolio, messaging, policy, and usage-observation routes; policy writes require a matching scope, expected revision, and recorded user-request reason.
...
- Agent control tokens are denied on every ordinary authenticated route, and issuance rejects unknown scopes. Main tokens permit portfolio reads, policy changes, usage observations, a bounded agent inventory, and provenance-preserving messages to another registered agent. Worker tokens permit only self-status reports. Neither grants owner access or general project, file, task, artifact, terminal, administrative, or account operations. `usage:write` means writing a timestamped observation record; it confers no provider-account mutation authority. Harness-native subagent threads inherit the parent runtime permission mode and may share its process environment; the compiled role-scope instruction forbids child-thread control use, but the PTY adapter cannot enforce a separate cryptographic identity for an internal harness thread.
