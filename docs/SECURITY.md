...
8. Behavior-control authority is issued only to a main-role agent. Its short-lived token is accepted only by dedicated policy and usage-observation routes; policy writes require a matching scope, expected revision, and recorded user-request reason.
9. Account authority is deliberately absent. The allowlist contains only policy and observation scopes; there is no route or helper action for token refreshes, rate-limit resets, credits, purchases, billing, plans, authentication, entitlements, or API-funded execution.
...
- Agent control tokens are denied on every ordinary authenticated route, and issuance rejects unknown scopes. The allowlist permits policy changes, usage observations, a bounded agent inventory, and provenance-preserving messages to another registered agent. It does not grant owner access or general project, file, task, artifact, terminal, administrative, or account operations. `usage:write` means writing a timestamped observation record; it confers no provider-account mutation authority. Independent WebSpider worker instances receive neither the helper environment nor a token. Harness-native subagent threads inherit the parent runtime permission mode and may share its process environment; the compiled role-scope instruction forbids child-thread control use, but the PTY adapter cannot enforce a separate cryptographic identity for an internal harness thread.
