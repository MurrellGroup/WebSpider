...
function renderWorkerInstructions(project, policy, customInstructions = '') {
  const lines = [
    ...
    '',
    'Persistent project Sub-Spider. User instructions arrive directly, are authoritative, and bypass the Master.',
    '`report --status working|blocked|completed --summary TEXT` records status. Add `--notify-master` only for delegated results or actionable blockers/risks/decisions. Never narrate routine direct work.',
    ...
    'Linked chat: use `chats list/read/send`; answer @mentions in-topic.',
...
function renderMainInstructions(project, policy, customInstructions = '') {
  const lines = [
    ...
    'Use judgment. Inspect first; ask only for material choices, missing authority, or unavailable essentials. Validate claims.',
